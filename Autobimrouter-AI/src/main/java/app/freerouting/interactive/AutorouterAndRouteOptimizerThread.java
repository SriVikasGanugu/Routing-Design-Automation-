...
public class AutorouterAndRouteOptimizerThread extends InteractiveActionThread {
  ...
  protected AutorouterAndRouteOptimizerThread(BoardHandling p_board_handling) {
    ...
    this.batch_autorouter = new BatchAutorouter(this, !autoroute_settings.get_with_fanout(), true,
    ...
    this.batch_opt_route = num_threads > 1
        ? new BatchOptRouteMT(this, num_threads, update_strategy, item_selection_strategy, hybrid_ratio)
  ...
  protected void thread_action() {
    ...
    try {
      ...
      boolean fanout_first = hdlg.get_settings().autoroute_settings.get_with_fanout()
      ...
      if (num_threads > 0) {
        FRLogger.info(
            "Starting route optimization on " + (num_threads == 1 ? "1 thread" : num_threads + " threads") + "...");
        ...
        double trace_length_before = hdlg.coordinate_transform
            .board_to_user(hdlg.get_routing_board().cumulative_trace_length());

        if (hdlg.get_settings().autoroute_settings.get_with_postroute() && !this.is_stop_requested()) {
          // String opt_message = tm.getText("batch_optimizer") + " " +
        ...
        double trace_length_after = hdlg.coordinate_transform
            .board_to_user(hdlg.get_routing_board().cumulative_trace_length());

        double percentage_improvement = (via_count_before != 0 && trace_length_before != 0)
        ...
        FRLogger
            .info("Route optimization was completed in " + FRLogger.formatDuration(routeOptimizationSecondsToComplete)
                + (percentage_improvement > 0
                    ? " and it improved the design by ~" + String.format("%(,.2f", percentage_improvement * 100.0) + "%"
                    : "")
                + ".");
    ...
    App.globalSettings.flag = true;
    System.out.println("DONE AUTOROUTING AND OPTIMIZING");
  }
  ...
  public void draw(Graphics p_graphics) {
    ...
    if (curr_air_line != null) {
      ...
      double draw_width = Math.min(this.hdlg.get_routing_board().communication.get_resolution(Unit.MIL) * 3, 300); // problem
                                                                                                                   // with
                                                                                                                   // low
                                                                                                                   // resolution
                                                                                                                   // on
