...
public class BatchOptRoute {
  ...
  /**
   * Calculates the cumulative trace lengths multiplied by the trace radius of all
   * traces on the
    ...
    for (;;) {
      ...
      if (curr_item instanceof Trace curr_trace) {
        ...
        if (fixed_state == FixedState.NOT_FIXED || fixed_state == FixedState.SHOVE_FIXED) {
          double weighted_trace_length = curr_trace.get_length() * (curr_trace.get_half_width() + p_board
  ...
  public void optimize_board(boolean save_intermediate_stages, float optimization_improvement_threshold,
    ...
    FRLogger.debug("Before optimize: Via count: " + routing_board.get_vias().size() + ", trace length: "
    ...
    while (((route_improved >= optimization_improvement_threshold) || (route_improved < 0))
      ...
      System.out.println(route_improved);
      System.out.println(optimization_improvement_threshold);
      ++curr_pass_no;
      ...
      if ((route_improved > optimization_improvement_threshold) && (save_intermediate_stages)) {
        ...
        // 2. To provide a way to check intermediate results for a long-running
        // optimization
   ...
   * routed board. Returns
   * the amount of improvements is made in percentage (expressed between 0.0 and
   * 1.0). -1 if the
  ...
  protected float opt_route_pass(int p_pass_no, boolean p_with_preferred_directions) {
    ...
    double trace_length_before = this.thread.hdlg.coordinate_transform
        .board_to_user(this.routing_board.cumulative_trace_length());
    // this.thread.hdlg.screen_messages.set_post_route_info(via_count_before,
    // trace_length_before, this.thread.hdlg.coordinate_transform.user_unit);
    ...
    String optimizationPassId = "BatchOptRoute.opt_route_pass #" + p_pass_no + " with " + via_count_before
        + " vias and " + String.format("%(,.2f", trace_length_before) + " trace length.";
    System.out.println(this.sorted_route_items);
    ...
    while (true) {
      ...
      if (opt_route_item(curr_item, p_pass_no, p_with_preferred_directions).improved()) {
        ...
        double trace_length_after = this.thread.hdlg.coordinate_transform
            .board_to_user(this.routing_board.cumulative_trace_length());

        route_improved = (float) ((via_count_before != 0 && trace_length_before != 0)
    ...
    FRLogger.info("Route optimizer pass #" + p_pass_no + " was completed in "
        + FRLogger.formatDuration(routeoptimizer_pass_duration));
    return route_improved;
  }
  ...
  protected ItemRouteResult opt_route_item(Item p_item, int p_pass_no, boolean p_with_preferred_directions) {
    ...
    String start_message = tm.getText("batch_optimizer") + " " + tm.getText("stop_message") + "        "
        + tm.getText("routeoptimizer_pass") + p_pass_no;
    // this.thread.hdlg.screen_messages.set_status_message(start_message); // assume
    // overwriting messages is harmless
    ...
    // no need to undo for cloned board which is either promoted to master or
    // discarded
    ...
    BatchAutorouter.autoroute_passes_for_optimizing_item(this.thread, MAX_AUTOROUTE_PASSES, ripup_costs,
        p_with_preferred_directions, this.clone_board ? this.routing_board : null);
    ...
    ItemRouteResult result = new ItemRouteResult(p_item.get_id_no(), via_count_before, via_count_after,
        this.min_cumulative_trace_length_before, trace_length_after, incomplete_count_before, incomplete_count_after);
    ...
    if (route_improved) {
      if (incomplete_count_after < incomplete_count_before
        ...
        // Catch unexpected increase of cumulative trace length somewhere for example by
        // removing
        // acid traps.
      ...
      double new_trace_length = this.thread.hdlg.coordinate_transform
          .board_to_user(this.routing_board.cumulative_trace_length());
      // this.thread.hdlg.screen_messages.set_post_route_info(via_count_after,
      // new_trace_length, this.thread.hdlg.coordinate_transform.user_unit);
    } else {
      if (!this.clone_board) {
   ...
   * public double length_reduced() { return trace_length_before -
   * trace_length_after; }
   * }
   */
   ...
   * and traces on the
   * board change while optimizing the item list of the board is read from scratch
   * each time the
