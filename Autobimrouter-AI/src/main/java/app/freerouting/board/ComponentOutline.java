...
public class ComponentOutline extends Item implements Serializable
{
  ...
  public void draw(Graphics p_g, GraphicsContext p_graphics_context, Color[] p_color_arr, double p_intensity)
  {
    ...
    // Retrieve the color and intensity for this layer
    ...
    // Determine the drawing width, ensure reasonable values for KiCad
    double draw_width = Math.min(this.board.communication.get_resolution(Unit.MIL), 100);

    // Optimized boundary drawing (path simplification and clipping applied in draw_boundary)
