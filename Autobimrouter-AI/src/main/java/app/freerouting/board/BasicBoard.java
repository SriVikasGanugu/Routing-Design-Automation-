...
public class BasicBoard implements Serializable
{
  ...
  {
    // Using ArrayList instead of LinkedList for faster random access and better iteration performance
    List<Item> result = new ArrayList<>();
//    Collection<Item> result = new LinkedList<>(); // original
    Iterator<UndoableObjects.UndoableObjectNode> it = item_list.start_read_object();
//    List<Item> result = StreamSupport.stream(Spliterators.spliteratorUnknownSize(it, Spliterator.ORDERED), true)
//            .map(item -> (Item) item_list.read_object(it))
//            .filter(Objects::nonNull)
//            .collect(Collectors.toList());
    // Instead of a for loop with a break condition, using a while loop for clarity
    while (it.hasNext()) {
      Item curr_item = (Item) item_list.read_object(it);

      // Skip null items (if read_object() might return null objects)
      if (curr_item != null) {
